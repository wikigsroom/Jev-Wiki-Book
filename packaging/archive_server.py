...
from jev_core.release_info import SERVER_PROGRAM, server_stem
BUILD = ROOT / "server_build"
bundle = BUILD / "dist" / SERVER_PROGRAM
target = BUILD / "packages"
target.mkdir(exist_ok=True)
system = platform.system()
name = server_stem(system)
...
if system == "Windows":
    ...
    with zipfile.ZipFile(archive, "w", zipfile.ZIP_DEFLATED, compresslevel=3) as output:
        for file in files:
            output.write(file, SERVER_PROGRAM + "/" + file.relative_to(bundle).as_posix())
else:
    archive = target / (name + ".tar.gz")
    with tarfile.open(archive, "w:gz", compresslevel=3) as output:
        output.add(bundle, arcname=SERVER_PROGRAM)
